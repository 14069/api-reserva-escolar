...
create index if not exists idx_bookings_resource_date_status on public.bookings (resource_id, booking_date, status);
...
select setval(
    ...
    greatest((select coalesce(max(id), 1) from public.notifications), 1),
    (select coalesce(max(id), 0) > 0 from public.notifications)
);
