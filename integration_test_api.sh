...
request POST "login" '{"school_code":"ETI001","email":"naoexiste@example.com","password":"senhaerrada"}'
assert_status 401
...
if [[ -n "$DIAGNOSTIC_TOKEN" ]]; then
  ...
  request GET "check-supabase-connection?diagnostic_token=${DIAGNOSTIC_TOKEN}"
  assert_status 200
...
request POST "login" "$(printf '{"school_code":"%s","email":"%s","password":"%s"}' "$TEST_SCHOOL_CODE" "$TEST_EMAIL" "$TEST_PASSWORD")"
assert_status 200
...
print_step "GET protegido /my-bookings"
request GET "my-bookings?school_id=${SCHOOL_ID}&user_id=${USER_ID}" "" "$AUTH_TOKEN"
assert_status 200
...
request GET "my-bookings?school_id=${SCHOOL_ID}&user_id=${USER_ID}"
assert_status 401
...
print_step "POST /bookings"
request POST "bookings" "$WRITE_BODY" "$AUTH_TOKEN"
assert_status 201
